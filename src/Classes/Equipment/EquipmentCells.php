...
class EquipmentCells extends ParentCells
{
    ...
    public function setCars(int $startRow, TaskItemInterface $equipment, EquipmentKit $equipmentKit)
    {
        ...
        $equipmentKit->getCards()->map(function (Card $card) use ($equipment, &$rowCount) {
            ...
            $this->sheet->setCellValue('F' . $rowCount, $card->getTaskCardOtherFieldsByTask($equipment)->getComment()); // Оборудование есть, проблема с меткой
            $this->sheet->setCellValue('G' . $rowCount, (int)$card->getAccounting()); // Учет/Инвентаризация
